...
def place_order(request, dic):
    ...
    for i in range(len(goodId)):
        ...
        orderdic = {'goods': goods, 'count': i + 1, 'sumtotal': goods.goodsPrice * int(count[i]),
                    'goodscount': count[i]}
        if len(cartId) > 0:
...
def place_hander(request, dic):
    ...
    if cartIdList[0]:
        for i in cartIdList:
            ...
            OrderDetail = orders.orderdetail_set.create(goodsName=goods.goodsName, goodsPrice=goods.goodsPrice,
                                                        buyCount=cart.buyCount, good_id_id=goods.id)
            OrderDetail.save()
            cart.isDelete = True
            cart.save()
    else:
        goods = Goods.objects.get(id=int(goodsId))
        orders.orderdetail_set.create(goodsName=goods.goodsName, goodsPrice=goods.goodsPrice, buyCount=int(goodscount),
                                      good_id_id=goods.id)
